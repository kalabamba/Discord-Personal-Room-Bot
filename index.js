  ...
  try {
    ...
    if (oldState.channelId !== newState.channelId && newState.channelId !== null && client.channels.cache.get(newState.channelId).name === process.env.joinRoomName && client.channels.cache.get(newState.channelId).parentId === client.channels.cache.get(newState.channelId).guild.channels.cache.find(channel => channel.name === process.env.categoryName && channel.type === ChannelType.GuildCategory).id ){
      ...
      if (channel.guild.channels.cache.find(channel => channel.name === newState.member.displayName + "'s Room" && channel.type === ChannelType.GuildVoice) === undefined){
        const category = channel.guild.channels.cache.find(channel => channel.name === process.env.categoryName && channel.type === ChannelType.GuildCategory);
      ...
      if (channel.name !== process.env.joinRoomName && channel.parentId === channel.guild.channels.cache.find(channel => channel.name === process.env.categoryName && channel.type === ChannelType.GuildCategory).id) {
        setTimeout(()=> {
          try{
